...
void create_instances()
{
    ...
    selectImg = (BITMAP *)gfxDat[SELECT_BMP].dat;
...
int _select_handler_(int inpSelectIndex, BITMAP *srcBmp, int limit)
{
    ...
    // setUp screenState savers
    clear_to_color(tempStorage, CBLACK);
    _save_reblit_buffer_state_(FULLSAV);

    _blit_translucent_bmp_(selectImg, srcBmp, 128, 280, (300 + SELECTGAP * inpSelectIndex));
    update_screen();
    ...
    while(contLoop)
    {
        while(keypressed())
        {
            ...
            switch(readkey() >> 8)
            {
                ...
                case KEY_ENTER:
                    retVal = inpSelectIndex;
                    contLoop = false;   // exit sequence
                break;
            }
        }
    }
